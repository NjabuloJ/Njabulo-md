...
} from "baileys-pro": "github:nstar-y/bail";
...
┊⌛︎ Invalid or missing SESSION_ID in .env
┗──────────────⊷`));
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(chalk.red(`┏──────────────⊷
┊⌛︎ Failed to load SESSION_ID: ${error.message}
┗──────────────⊷`));
    process.exit(1);
  }
}
  ...
  try {
    ...
    const Matrix = makeWASocket({
      ...
      getMessage: async (key) => {
        ...
        return { conversation: "Njabulo-md whatsapp user bot" };
      },
    });
      ...
      if (connection === "close") {
        ...
        switch (statusCode) {
          case DisconnectReason.badSession:
            console.error(chalk.red(`┏──────────────⊷
┊⌛︎ Invalid session, update SESSION_ID in .env
┗──────────────⊷`));
            process.exit();
            break;
          ...
          case DisconnectReason.loggedOut:
            console.error(chalk.red(`┏──────────────⊷
┊⌛︎ Logged out, update SESSION_ID in .env
┗──────────────⊷`));
...
┗──────────────⊷`));
    ...
    console.error(chalk.red(`┏──────────────⊷
┊⌛︎ Critical Error: ${error.message}
┗──────────────⊷`));
    process.exit(1);
  }
}
...
app.get("/", (req, res) => {
  res.send("Njabulo-md is running!");
});
